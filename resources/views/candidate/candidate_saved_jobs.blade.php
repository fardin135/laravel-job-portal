            ...
            <tbody class="text-center">
                ...
                @else
                    @foreach ($savedJobs as $savedJob)
                        <tr class="table-warning">
                            <td>{{ $savedJob->job->job_title }}</td>
                                <td>{{ $savedJob->created_at->diffForHumans() }}</td>
                                    <td>
                                        <a href="{{ route('show_job_details' , $savedJob->job_id) }}" class="btn btn-sm btn-info">View</a>
                                            ...
                                            <button type="button" class="btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#confirmationModal">Delete</button>
                                                ...
                                                <div class="modal fade" id="confirmationModal" tabindex="-1" aria-labelledby="confirmationModalLabel" aria-hidden="true">
                                                    ...
                                                    <form id="deleteForm" method="POST" action="{{ route('jobs.savedJobDestroy') }}">
                                                        @csrf
                                                            @method('DELETE')
